	...
	if eventConfig.CustomCutsEnabled && p.server.GetSessionInfo().SessionType == acserver.SessionTypeRace {
		...
		if penaltyInfo.clearPenaltyIn > 0 {
			...
			if penaltyInfo.clearPenaltyIn == 0 {
				...
				if err != nil {
					...
					err = p.server.SendChat("Your penalty BoP has been cleared", acserver.ServerCarID, entrant.CarID, false)
		...
		if ((penaltyInfo.totalCleanLaps == 0 && !eventConfig.CustomCutsOnlyIfCleanSet) || averageCleanLap > lap.LapTime.Milliseconds()) && lap.Cuts > 0 {
			...
			if penaltyInfo.warnings > eventConfig.CustomCutsNumWarnings {
				...
				err = p.server.SendChat(chatMessage, acserver.ServerCarID, entrant.CarID, false)
				...
				err = p.server.SendChat(fmt.Sprintf("You cut the track %d times this lap and gained time! (warning %d/%d)", lap.Cuts, penaltyInfo.warnings, eventConfig.CustomCutsNumWarnings), acserver.ServerCarID, entrant.CarID, false)
